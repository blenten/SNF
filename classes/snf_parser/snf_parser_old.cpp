#include "snf_parser_old.h"

FunctionType SNF_ParserOld::parse(std::string input, Expression &output)
{   
     expression=&output;
    _input=input;
    initialChecking();
...
void SNF_ParserOld::initialChecking()
{
    expression->clear();
    variables.clear();
    removeUnused();
...
void SNF_ParserOld::removeUnused()
{
    size_t i=0;
...
void SNF_ParserOld::checkBrackets()
{
...
bool SNF_ParserOld::isFunctionSeemsInShortForm()
{
     if (!ShortFormConverter::checkOperation(_input)) return false;
     return true;
}

FunctionType SNF_ParserOld::parseShortForm()
{
...
FunctionType SNF_ParserOld::parseExpandedForm()
{
    removeCommas();
...
void SNF_ParserOld::removeCommas()
{
    size_t i=0;
    while (getSymbolType(_input[i])!=SYMBOL_ZERO)
    {
...
void SNF_ParserOld::insertConjunctionSymbols()
{
    int len=_input.size();
...
void SNF_ParserOld::checkInversions()
{
...
FunctionType SNF_ParserOld::getVariables()
{
     variables.clear();
     std::string currVar="";
     size_t lena=0;
...
size_t SNF_ParserOld::increaseIndexToVariable(size_t index)
{
...
std::string SNF_ParserOld::getOperand (size_t &index)
{
     std::string output;
...
OperationState SNF_ParserOld::getNextState(size_t & lena, OperationState prevState)
{
...
OperationState SNF_ParserOld::getOperationStateAfterLBracket(size_t index, OperationState currState)
{
    getOperand(index);
...
bool SNF_ParserOld::isVariablesRepeat()
{
...
void SNF_ParserOld::fillExpressionVector(const FunctionType& ft)
{
...
void SNF_ParserOld::addOperandToExpression ()
{
    Operand op;
    expression->push_back(op);
}

Variable SNF_ParserOld::parseVariable(std::string &currVar)
{
    Variable var;
